...
const AnalogToDpad = ({
	...
	const applySquareness = function (x, y, squareness, deadzone) {
		const deadzone_scale =
			deadzone > 0 ? deadzone / Math.pow(deadzone, 1 + squareness) : 1.0;

		return [
			Math.pow(Math.abs(x), 1 + squareness) * (x > 0 ? 1 : -1) * deadzone_scale,
	...
	useEffect(() => {
		...
		function colorFunction(x: number, y: number) {
			...
			const cardinal = function (value, other_value, in_debounce) {
		...
		renderCanvas(
			values.AnalogToDpadInputEnabled,
			canvas8WayRef.current,
			colorFunction,
		);
	}, [
		canvas8WayRef.current,
		values.AnalogToDpadInputEnabled,
		values.analogToDpad8WaySquareness,
		values.analogToDpad8WayDeadzone,
		values.analogToDpad8WayDebounce,
		values.analogToDpad8WaySlope,
		values.analogToDpad8WayOffset,
	]);
	...
	useEffect(() => {
		...
		function colorFunction(x: number, y: number) {
			...
			const cardinal = function (debounce_x, debounce_y) {
				// 4-way mode will only ever have one of these active at a time
				const debounced_deadzone = Math.max(
					0,
					deadzone - (debounce_x + debounce_y),
				);
				...
				if (
					dist_squared > debounced_deadzone * debounced_deadzone &&
					(Math.abs(x) > offset_x || Math.abs(y) > offset_y)
				) {
			...
			const nx = cardinal(debounce, 0)[0];
			const ny = cardinal(0, debounce)[1];
		...
		renderCanvas(
			values.AnalogToDpadInputEnabled,
			canvas4WayRef.current,
			colorFunction,
		);
	}, [
		canvas4WayRef.current,
		values.AnalogToDpadInputEnabled,
		values.analogToDpad4WaySquareness,
		values.analogToDpad4WayDeadzone,
		values.analogToDpad4WayDebounce,
		values.analogToDpad4WayOffset,
	]);
					...
					>
						...
						>
							<Row>
								<Col lg={4}>
									<center>
										<canvas ref={canvas8WayRef} width="300" height="300" />
									</center>
								</Col>
								<Col>
									<Row className="mb-3">
						...
						>
							<Row>
								<Col lg={4}>
									<center>
										<canvas ref={canvas4WayRef} width="300" height="300" />
									</center>
								</Col>
								<Col>
									<Row className="mb-3">
